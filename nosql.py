    ...
    if action == "show" and len(tokens) == 2 and tokens[1].lower() == "databases":
        ...
        try:
            ...
            if not isinstance(current_db, dict) or not current_db:
                return "Error: Database is empty or has an invalid structure for export."
    
            downloads_folder = get_downloads_directory()
            file_path = os.path.join(downloads_folder, f"{file_name}.{file_format.lower()}")
    
            # ---------------- JSON (already NoSQL document shape) ---------------
            if file_format == "JSON":
                with open(file_path, "w", encoding="utf-8") as f:
                    json.dump(current_db, f, indent=4, default=str)
                ...
                with open(file_path, "w", newline="", encoding="utf-8") as csv_file:
                    ...
                    for table_name, records in current_db.items():
                        ...
                        if records:
                            ...
                            # since NoSQL records aren't guaranteed to match.
                            columns = []
            ...
            # ---------------- SQL (inferred schema, real dump) -------------------
                ...
                sql_lines = [f"-- Export of database '{db_name}' generated by xdb (NoSQL)", ""]
    
                for table_name, records in current_db.items():
                    if not records:
                        continue
    
                    # Union of keys across all records defines the table's columns
                    columns = []
                    for record in records:
                        for key in record.keys():
                            if key not in columns:
                    ...
                    # Infer each column's type from the first record that has a
                    # non-null value for it; default to TEXT if never populated.
                    col_types = {}
                    for col in columns:
                        col_types[col] = "TEXT"
                        for record in records:
                            if record.get(col) is not None:
                                col_types[col] = infer_sql_type(record[col])
                                break
                    ...
                    for record in records:
                        ...
                        sql_lines.append(
                            f"INSERT INTO `{table_name}` ({col_list}) VALUES ({', '.join(formatted_values)});"
                        )
                    sql_lines.append("")
