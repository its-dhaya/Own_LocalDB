    ...
    # SELECT DATA
        ...
        try:
            ...
            if len(parts) != 6 or parts[2].lower() != "as" or parts[4].lower() != "in":
                return "Invalid export command format. Use: EXPORT database_name AS file_name IN [CSV/JSON/SQL]"
    
            db_name, file_name, file_format = parts[1], parts[3], parts[5].upper()
            if file_format not in ["CSV", "JSON", "SQL"]:
                return "Error: Unsupported file format. Use CSV, JSON, or SQL."
    
            downloads_folder = get_downloads_directory()
            file_path = os.path.join(downloads_folder, f"{file_name}.{file_format.lower()}")
    
            # ---------------- JSON (NoSQL-ready document shape) ----------------
            if file_format == "JSON":
                if not isinstance(current_db, dict) or not current_db:
                    return "Error: Database is empty or has an invalid structure for JSON export."
                ...
                for table_name, table_info in current_db.items():
                    if not isinstance(table_info, dict) or "columns" not in table_info or "data" not in table_info:
                        continue
                    columns = table_info["columns"]
                    # One document per row, keyed by column name -> matches how a
                    # NoSQL collection stores records, so this can be inserted
                    # directly (e.g. db.<table_name>.insertMany(documents)).
                    documents = [dict(zip(columns, row)) for row in table_info["data"]]
                    nosql_style[table_name] = documents
    
                with open(file_path, "w", encoding="utf-8") as f:
                    json.dump(nosql_style, f, indent=4, default=str)
                return f"Database '{db_name}' successfully exported as NoSQL-ready JSON! File saved at: {file_path}"
            ...
            # ---------------- SQL (real CREATE TABLE + INSERT dump) -------------
            elif file_format == "SQL":
                if not isinstance(current_db, dict) or not current_db:
                    return "Error: Database is empty or has an invalid structure for SQL export."
    
                # Map this engine's internal types onto standard SQL column types.
                type_map = {
                    "INT": "INT",
                    "FLOAT": "FLOAT",
                    "TEXT": "TEXT",
                ...
                sql_lines = [f"-- Export of database '{db_name}' generated by xdb", ""]
    
                for table_name, table_info in current_db.items():
                    if not isinstance(table_info, dict) or "columns" not in table_info:
                        continue
                    columns = table_info["columns"]
                    col_types = table_info["types"]
                    data = table_info.get("data", [])
    
                    # CREATE TABLE
                    col_defs = ", ".join(
                        f"`{col}` {type_map.get(col_types.get(col, 'TEXT'), 'TEXT')}"
                        for col in columns
                    )
                    sql_lines.append(f"DROP TABLE IF EXISTS `{table_name}`;")
                    sql_lines.append(f"CREATE TABLE `{table_name}` ({col_defs});")
                    sql_lines.append("")
    
                    # INSERT INTO (one statement per row; batch if you prefer)
                    ...
                    for row in data:
                        ...
                        sql_lines.append(
                            f"INSERT INTO `{table_name}` ({col_list}) VALUES ({values_clause});"
                        )
                    sql_lines.append("")  # blank line between tables
    
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write("\n".join(sql_lines))
                return f"Database '{db_name}' successfully exported as SQL! File saved at: {file_path}"
